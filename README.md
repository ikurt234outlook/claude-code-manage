...
- **加密**：根密钥 `.master_key`（32 字节随机，AES-256-GCM）以 `0600` 权限存放；每个环境的整份 settings JSON 加密为 `encrypted_settings` 落入 `profiles.json`，该持久化主数据源中**不含明文 Token**。
- **JIT**：启动时才把选中环境解密为 `settings/<id>.json`（同样强制 `0600`），启动前会先清理上次残留的 `settings/*.json`，随后交给 `claude`。
...
- 静态存储仅含密文；明文 Token 会在当前启动所需的 `0600` 权限 JIT 文件里短暂落盘。
- 每次启动前都会清理旧的 `settings/*.json`，以减少历史明文 settings 残留。
